...
def generate_order_number():
    ...
    Ejemplo: ORD-000001
    """
    last_order = orders.find_one(sort=[("date", -1)])
...
def create_order():
    try:
        ...
        # Simulación de pago
        payment_approved = random.random() < 0.85
        order_number = generate_order_number()

        order_data = {
            "order_number": order_number,
            "cart_id": cart_id,  # MEJORA: trazabilidad
            "customer": {
...
def get_order_stats():
    total_revenue_cursor = orders.aggregate([
        {"$match": {"status": "completed"}},
        {"$group": {"_id": None, "total": {"$sum": "$total"}}}
    ])

    revenue_list = list(total_revenue_cursor)
...
def delete_order(id):
    try:
        ...
        # OPCIONAL: proteger órdenes completadas
